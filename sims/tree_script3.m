...
data = load('gurobi.txt');
datautility = load('glplotdata.txt');
datademand = load('glplotdata.txt');
dataratio = load('glplotdata.txt');
datakhaled = load('glplotdata.txt');
strive = load('glplotdata.txt');
...
ym(i) = (mu - lower);
yp(i) = (upper - mu);
gu_ym(i) = (gu_mu - gu_lower)/4;
gu_yp(i) = (gu_upper - gu_mu)/4;
...
set(h2,'MarkerSize',15,'Marker','.','DisplayName','GLA',...
...
set(h1,'MarkerSize',15,'Marker','.','DisplayName','GUROBI OPTF MAX',...
...
%set(h3,'MarkerSize',10,'Marker','o','DisplayName','GMA',...
...
%set(h2,'MarkerSize',15,'Marker','.','DisplayName','GDA',...
    %'Color',[0 0 1],'LineWidth',1);
%hold off;
%hold on;
...
% Create ylabel
ylabel('Maximized valuation','FontSize',12);

% Create title
%title('maxPA for scenario FUM','FontSize',14);

% Create legend
legend1 = legend({'GLA','GUROBI OPTF MAX'});
